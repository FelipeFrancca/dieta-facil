...
export const MultiSelectSummary = ({ selectedFoods, quantities, onQuantityChange }) => {
  // Calcular totais
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Typography variant="body2" color="textSecondary">
                      {Math.round(food.calorias * factor)} kcal | 
                      P: {Math.round(food.proteina * factor)}g | 
                      C: {Math.round(food.carbo * factor)}g | 
                      G: {Math.round(food.gordura * factor)}g
                    </Typography>
                  </Box>
                  <TextField
                    label="Quantidade (g)"
                    type="number"
                    value={quantity}
                    onChange={(e) => onQuantityChange(foodKey, Number(e.target.value))}
                    size="small"
                    sx={{ width: 120 }}
