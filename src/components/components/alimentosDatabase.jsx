export const alimentosDatabase = {
  // === CEREAIS E GRÃOS ===
  "arroz integral": { 
    calorias: 123, proteina: 2.6, carbo: 23, gordura: 0.9,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 160 },
      { tipo: "escumadeira", descricao: "Escumadeira", pesoPorUnidade: 45 }
    ]
  },
  "arroz branco": { 
    calorias: 130, proteina: 2.7, carbo: 28, gordura: 0.3,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 160 },
      { tipo: "escumadeira", descricao: "Escumadeira", pesoPorUnidade: 45 }
    ]
  },
  "arroz integral cozido": { 
    calorias: 124, proteina: 2.6, carbo: 25, gordura: 1.0,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 },
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 80 },
      { tipo: "escumadeira", descricao: "Escumadeira", pesoPorUnidade: 60 }
    ]
  },
  "arroz branco cozido": { 
    calorias: 128, proteina: 2.4, carbo: 26, gordura: 0.2,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 },
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 80 },
      { tipo: "escumadeira", descricao: "Escumadeira", pesoPorUnidade: 60 }
    ]
  },
  "arroz parboilizado": { calorias: 348, proteina: 7.4, carbo: 78, gordura: 1.0 },
  "aveia": { 
    calorias: 389, proteina: 17, carbo: 66, gordura: 7,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 10 },
      { tipo: "colher_cha", descricao: "Colher de chá", pesoPorUnidade: 3 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 80 }
    ]
  },
  "aveia em flocos": { 
    calorias: 394, proteina: 13.9, carbo: 60.8, gordura: 8.5,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 8 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 80 }
    ]
  },
  "quinoa": { 
    calorias: 368, proteina: 14, carbo: 64, gordura: 6,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 12 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 180 }
    ]
  },
  "quinoa cozida": { 
    calorias: 120, proteina: 4.4, carbo: 22, gordura: 1.9,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 18 },
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 90 }
    ]
  },
  "cuscuz de milho": { calorias: 112, proteina: 3.5, carbo: 23, gordura: 0.2 },
  "cuscuz cru": { calorias: 358, proteina: 11.2, carbo: 77, gordura: 0.8 },
  "farinha de trigo": { 
    calorias: 364, proteina: 10.3, carbo: 75, gordura: 0.9,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 10 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 120 }
    ]
  },
  "farinha de mandioca": { 
    calorias: 365, proteina: 1.8, carbo: 85, gordura: 0.8,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 12 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 150 }
    ]
  },
  "farinha de milho": { calorias: 365, proteina: 7.3, carbo: 79, gordura: 2.8 },
  "polenta": { calorias: 70, proteina: 1.6, carbo: 16, gordura: 0.2 },
  "tapioca": { calorias: 98, proteina: 0.2, carbo: 25, gordura: 0.0 },

  // === LEGUMINOSAS ===
  "feijão preto": { 
    calorias: 132, proteina: 8.9, carbo: 24, gordura: 0.5,
    unidades: [
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 190 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 }
    ]
  },
  "feijão carioca": { 
    calorias: 137, proteina: 8.5, carbo: 25, gordura: 0.6,
    unidades: [
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 190 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 }
    ]
  },
  "feijão preto cozido": { 
    calorias: 77, proteina: 4.5, carbo: 14, gordura: 0.5,
    unidades: [
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 100 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 }
    ]
  },
  "feijão carioca cozido": { 
    calorias: 76, proteina: 4.8, carbo: 13.6, gordura: 0.5,
    unidades: [
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 100 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 }
    ]
  },
  "feijão branco": { calorias: 123, proteina: 8.2, carbo: 21, gordura: 0.9 },
  "lentilha": { 
    calorias: 116, proteina: 9, carbo: 20, gordura: 0.4,
    unidades: [
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 200 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 }
    ]
  },
  "lentilha cozida": { 
    calorias: 93, proteina: 6.8, carbo: 16, gordura: 0.4,
    unidades: [
      { tipo: "concha", descricao: "Concha média", pesoPorUnidade: 110 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 18 }
    ]
  },
  "grão de bico": { calorias: 164, proteina: 8.9, carbo: 27, gordura: 2.6 },
  "grão de bico cozido": { calorias: 121, proteina: 6.3, carbo: 19.3, gordura: 2.1 },
  "ervilha": { calorias: 81, proteina: 5.4, carbo: 14, gordura: 0.4 },
  "ervilha seca": { calorias: 341, proteina: 21.6, carbo: 60, gordura: 2.0 },
  "soja": { calorias: 329, proteina: 30, carbo: 24, gordura: 15 },

  // === CARNES BOVINAS ===
  "carne bovina magra": { calorias: 250, proteina: 26, carbo: 0, gordura: 15 },
  "patinho cru": { calorias: 108, proteina: 21.8, carbo: 0, gordura: 2.2 },
  "patinho cozido": { calorias: 163, proteina: 32.8, carbo: 0, gordura: 3.4 },
  "patinho grelhado": { calorias: 219, proteina: 32.4, carbo: 0, gordura: 9.2 },
  "alcatra crua": { calorias: 130, proteina: 21.4, carbo: 0, gordura: 4.6 },
  "alcatra grelhada": { calorias: 216, proteina: 31.9, carbo: 0, gordura: 9.2 },
  "coxão mole cru": { calorias: 120, proteina: 20.7, carbo: 0, gordura: 3.9 },
  "coxão mole cozido": { calorias: 177, proteina: 31.2, carbo: 0, gordura: 5.8 },
  "contrafilé cru": { calorias: 138, proteina: 21.6, carbo: 0, gordura: 5.4 },
  "contrafilé grelhado": { calorias: 222, proteina: 31.4, carbo: 0, gordura: 10.1 },
  "carne moída crua": { calorias: 136, proteina: 20.2, carbo: 0, gordura: 6.0 },
  "carne moída refogada": { calorias: 232, proteina: 26.1, carbo: 0, gordura: 13.4 },
  "picanha crua": { calorias: 178, proteina: 20.5, carbo: 0, gordura: 10.2 },
  "picanha grelhada": { calorias: 298, proteina: 28.1, carbo: 0, gordura: 20.0 },

  // === AVES ===
  "frango grelhado": { calorias: 165, proteina: 31, carbo: 0, gordura: 3.6 },
  "peito de frango": { calorias: 165, proteina: 31, carbo: 0, gordura: 3.6 },
  "peito de frango cru": { calorias: 100, proteina: 22.3, carbo: 0, gordura: 1.2 },
  "peito de frango cozido": { calorias: 159, proteina: 32.0, carbo: 0, gordura: 3.1 },
  "peito de frango grelhado": { calorias: 148, proteina: 32.8, carbo: 0, gordura: 1.8 },
  "coxa de frango crua": { calorias: 127, proteina: 18.3, carbo: 0, gordura: 5.7 },
  "coxa de frango cozida": { calorias: 204, proteina: 27.3, carbo: 0, gordura: 9.7 },
  "sobrecoxa crua": { calorias: 115, proteina: 19.8, carbo: 0, gordura: 3.7 },
  "sobrecoxa cozida": { calorias: 175, proteina: 26.0, carbo: 0, gordura: 7.8 },
  "frango inteiro cru": { calorias: 143, proteina: 19.3, carbo: 0, gordura: 6.9 },
  "frango assado": { calorias: 219, proteina: 27.3, carbo: 0, gordura: 11.2 },
  "peru": { calorias: 104, proteina: 20.1, carbo: 0, gordura: 2.4 },

  // === PEIXES E FRUTOS DO MAR ===
  "peixe tilápia": { calorias: 128, proteina: 26, carbo: 0, gordura: 2.7 },
  "tilápia crua": { calorias: 96, proteina: 20.1, carbo: 0, gordura: 1.7 },
  "tilápia grelhada": { calorias: 129, proteina: 26.9, carbo: 0, gordura: 2.0 },
  "salmão": { calorias: 208, proteina: 22, carbo: 0, gordura: 13 },
  "salmão cru": { calorias: 172, proteina: 25.4, carbo: 0, gordura: 7.3 },
  "salmão grelhado": { calorias: 216, proteina: 27.6, carbo: 0, gordura: 11.2 },
  "atum": { calorias: 109, proteina: 25.0, carbo: 0, gordura: 0.8 },
  "sardinha": { calorias: 135, proteina: 24.6, carbo: 0, gordura: 4.1 },
  "bacalhau": { calorias: 135, proteina: 29.1, carbo: 0, gordura: 1.2 },
  "camarão": { calorias: 91, proteina: 18.1, carbo: 1.5, gordura: 1.2 },
  "lagosta": { calorias: 77, proteina: 16.9, carbo: 0, gordura: 0.9 },

  // === SUÍNOS ===
  "lombo suíno cru": { calorias: 143, proteina: 21.5, carbo: 0, gordura: 6.0 },
  "lombo suíno assado": { calorias: 221, proteina: 29.9, carbo: 0, gordura: 10.2 },
  "costela suína": { calorias: 290, proteina: 18.3, carbo: 0, gordura: 24.1 },
  "pernil suíno": { calorias: 198, proteina: 24.5, carbo: 0, gordura: 10.5 },
  "bacon": { calorias: 665, proteina: 8.1, carbo: 0.7, gordura: 69.3 },
  "linguiça": { calorias: 296, proteina: 17.1, carbo: 0.6, gordura: 24.9 },
  "presunto": { calorias: 162, proteina: 19.8, carbo: 0.8, gordura: 8.4 },

  // === OVOS (DIFERENTES PREPAROS) ===
  "ovo cozido": { 
    calorias: 155, proteina: 13, carbo: 1.1, gordura: 11,
    unidades: [
      { tipo: "unidade", descricao: "Ovo médio", pesoPorUnidade: 50 },
      { tipo: "unidade_grande", descricao: "Ovo grande", pesoPorUnidade: 60 }
    ]
  },
  "ovo cru": { 
    calorias: 143, proteina: 12.6, carbo: 0.7, gordura: 9.5,
    unidades: [
      { tipo: "unidade", descricao: "Ovo médio", pesoPorUnidade: 50 },
      { tipo: "unidade_grande", descricao: "Ovo grande", pesoPorUnidade: 60 }
    ]
  },
  "ovo frito": { 
    calorias: 196, proteina: 13.6, carbo: 0.6, gordura: 15.3,
    unidades: [
      { tipo: "unidade", descricao: "Ovo médio", pesoPorUnidade: 50 },
      { tipo: "unidade_grande", descricao: "Ovo grande", pesoPorUnidade: 60 }
    ]
  },
  "clara de ovo": { 
    calorias: 48, proteina: 10.9, carbo: 0.4, gordura: 0.0,
    unidades: [
      { tipo: "unidade", descricao: "Clara de 1 ovo", pesoPorUnidade: 35 }
    ]
  },
  "gema de ovo": { 
    calorias: 353, proteina: 16.1, carbo: 0.6, gordura: 31.1,
    unidades: [
      { tipo: "unidade", descricao: "Gema de 1 ovo", pesoPorUnidade: 15 }
    ]
  },
  "ovo pochê": { 
    calorias: 143, proteina: 12.6, carbo: 0.7, gordura: 9.5,
    unidades: [
      { tipo: "unidade", descricao: "Ovo médio", pesoPorUnidade: 50 }
    ]
  },
  "ovo mexido": { calorias: 154, proteina: 10.4, carbo: 1.6, gordura: 11.2 },
  "omelete": { calorias: 154, proteina: 10.8, carbo: 1.6, gordura: 11.2 },

  // === TUBÉRCULOS ===
  "batata doce": { calorias: 86, proteina: 1.6, carbo: 20, gordura: 0.1 },
  "batata doce cozida": { calorias: 77, proteina: 1.3, carbo: 18.4, gordura: 0.1 },
  "batata doce assada": { calorias: 90, proteina: 2.0, carbo: 20.7, gordura: 0.1 },
  "batata inglesa": { calorias: 77, proteina: 2, carbo: 17, gordura: 0.1 },
  "batata inglesa cozida": { calorias: 52, proteina: 1.4, carbo: 11.5, gordura: 0.1 },
  "batata inglesa frita": { calorias: 274, proteina: 3.8, carbo: 35.7, gordura: 13.2 },
  "mandioca": { calorias: 125, proteina: 1.2, carbo: 30.1, gordura: 0.3 },
  "mandioca cozida": { calorias: 96, proteina: 0.8, carbo: 23.0, gordura: 0.2 },
  "inhame": { calorias: 97, proteina: 2.0, carbo: 23, gordura: 0.1 },
  "cará": { calorias: 116, proteina: 2.8, carbo: 28, gordura: 0.1 },

  // === VEGETAIS E HORTALIÇAS ===
  "brócolis": { calorias: 34, proteina: 2.8, carbo: 7, gordura: 0.4 },
  "brócolis cozido": { calorias: 20, proteina: 1.9, carbo: 4.0, gordura: 0.2 },
  "espinafre": { calorias: 23, proteina: 2.9, carbo: 3.6, gordura: 0.4 },
  "espinafre cozido": { calorias: 15, proteina: 2.2, carbo: 1.6, gordura: 0.2 },
  "abobrinha": { calorias: 17, proteina: 1.2, carbo: 3.1, gordura: 0.3 },
  "abobrinha refogada": { calorias: 29, proteina: 1.6, carbo: 4.3, gordura: 1.0 },
  "cenoura": { calorias: 41, proteina: 0.9, carbo: 10, gordura: 0.2 },
  "cenoura cozida": { calorias: 22, proteina: 0.6, carbo: 5.1, gordura: 0.1 },
  "tomate": { calorias: 18, proteina: 0.9, carbo: 3.9, gordura: 0.2 },
  "alface": { calorias: 15, proteina: 1.4, carbo: 2.9, gordura: 0.2 },
  "pepino": { calorias: 16, proteina: 0.7, carbo: 4, gordura: 0.1 },
  "cebola": { calorias: 40, proteina: 1.1, carbo: 9.3, gordura: 0.1 },
  "alho": { calorias: 113, proteina: 5.1, carbo: 23.9, gordura: 0.2 },
  "pimentão": { calorias: 31, proteina: 1.0, carbo: 7.3, gordura: 0.2 },
  "repolho": { calorias: 27, proteina: 1.3, carbo: 6.1, gordura: 0.2 },
  "couve": { calorias: 29, proteina: 2.9, carbo: 5.1, gordura: 0.5 },
  "couve-flor": { calorias: 25, proteina: 2.0, carbo: 5.0, gordura: 0.3 },
  "berinjela": { calorias: 25, proteina: 1.0, carbo: 6.0, gordura: 0.2 },
  "quiabo": { calorias: 33, proteina: 1.9, carbo: 7.5, gordura: 0.2 },

  // === FRUTAS ===
  "banana": { 
    calorias: 89, proteina: 1.1, carbo: 23, gordura: 0.3,
    unidades: [
      { tipo: "unidade", descricao: "Banana média", pesoPorUnidade: 86 },
      { tipo: "unidade_pequena", descricao: "Banana pequena", pesoPorUnidade: 65 },
      { tipo: "unidade_grande", descricao: "Banana grande", pesoPorUnidade: 120 }
    ]
  },
  "maçã": { 
    calorias: 52, proteina: 0.3, carbo: 14, gordura: 0.2,
    unidades: [
      { tipo: "unidade", descricao: "Maçã média", pesoPorUnidade: 138 },
      { tipo: "fatia", descricao: "Fatia média", pesoPorUnidade: 25 }
    ]
  },
  "laranja": { 
    calorias: 47, proteina: 0.9, carbo: 12, gordura: 0.1,
    unidades: [
      { tipo: "unidade", descricao: "Laranja média", pesoPorUnidade: 154 },
      { tipo: "gomo", descricao: "Gomo grande", pesoPorUnidade: 15 }
    ]
  },
  "manga": { 
    calorias: 60, proteina: 0.8, carbo: 15, gordura: 0.4,
    unidades: [
      { tipo: "unidade", descricao: "Manga média", pesoPorUnidade: 200 },
      { tipo: "fatia", descricao: "Fatia média", pesoPorUnidade: 40 }
    ]
  },
  "morango": { 
    calorias: 32, proteina: 0.7, carbo: 7.7, gordura: 0.3,
    unidades: [
      { tipo: "unidade", descricao: "Morango médio", pesoPorUnidade: 12 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 150 }
    ]
  },
  "uva": { 
    calorias: 69, proteina: 0.7, carbo: 18, gordura: 0.2,
    unidades: [
      { tipo: "unidade", descricao: "Uva grande", pesoPorUnidade: 6 },
      { tipo: "cacho", descricao: "Cacho pequeno", pesoPorUnidade: 80 }
    ]
  },
  "melancia": { 
    calorias: 30, proteina: 0.6, carbo: 8, gordura: 0.2,
    unidades: [
      { tipo: "fatia", descricao: "Fatia média", pesoPorUnidade: 280 },
      { tipo: "xicara", descricao: "Xícara em cubos", pesoPorUnidade: 152 }
    ]
  },
  "abacaxi": { 
    calorias: 50, proteina: 0.5, carbo: 13, gordura: 0.1,
    unidades: [
      { tipo: "fatia", descricao: "Fatia média", pesoPorUnidade: 84 },
      { tipo: "xicara", descricao: "Xícara em cubos", pesoPorUnidade: 165 }
    ]
  },
  "mamão": { 
    calorias: 40, proteina: 0.5, carbo: 10.4, gordura: 0.1,
    unidades: [
      { tipo: "fatia", descricao: "Fatia média", pesoPorUnidade: 140 },
      { tipo: "xicara", descricao: "Xícara em cubos", pesoPorUnidade: 140 }
    ]
  },
  "pêra": { 
    calorias: 57, proteina: 0.4, carbo: 15.2, gordura: 0.1,
    unidades: [
      { tipo: "unidade", descricao: "Pêra média", pesoPorUnidade: 178 }
    ]
  },
  "kiwi": { 
    calorias: 61, proteina: 1.1, carbo: 14.7, gordura: 0.5,
    unidades: [
      { tipo: "unidade", descricao: "Kiwi médio", pesoPorUnidade: 69 }
    ]
  },
  "abacate": { 
    calorias: 160, proteina: 2, carbo: 9, gordura: 15,
    unidades: [
      { tipo: "unidade", descricao: "Abacate médio", pesoPorUnidade: 200 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 }
    ]
  },
  "coco": { calorias: 354, proteina: 3.3, carbo: 15, gordura: 33 },
  "açaí": { 
    calorias: 58, proteina: 0.8, carbo: 6.2, gordura: 3.9,
    unidades: [
      { tipo: "tigela", descricao: "Tigela pequena", pesoPorUnidade: 200 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 20 }
    ]
  },

  // === LATICÍNIOS ===
  "leite desnatado": { 
    calorias: 34, proteina: 3.4, carbo: 5, gordura: 0.1,
    unidades: [
      { tipo: "copo", descricao: "Copo (200ml)", pesoPorUnidade: 200 },
      { tipo: "xicara", descricao: "Xícara (240ml)", pesoPorUnidade: 240 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 }
    ]
  },
  "leite integral": { 
    calorias: 61, proteina: 2.9, carbo: 4.3, gordura: 3.5,
    unidades: [
      { tipo: "copo", descricao: "Copo (200ml)", pesoPorUnidade: 200 },
      { tipo: "xicara", descricao: "Xícara (240ml)", pesoPorUnidade: 240 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 15 }
    ]
  },
  "iogurte grego": { 
    calorias: 59, proteina: 10, carbo: 3.6, gordura: 0.4,
    unidades: [
      { tipo: "pote", descricao: "Pote individual (170g)", pesoPorUnidade: 170 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 25 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 245 }
    ]
  },
  "iogurte natural": { 
    calorias: 51, proteina: 4.1, carbo: 4.7, gordura: 1.5,
    unidades: [
      { tipo: "pote", descricao: "Pote individual (170g)", pesoPorUnidade: 170 },
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 25 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 245 }
    ]
  },
  "queijo cottage": { 
    calorias: 98, proteina: 11, carbo: 3.4, gordura: 4.3,
    unidades: [
      { tipo: "colher_sopa", descricao: "Colher de sopa", pesoPorUnidade: 25 },
      { tipo: "xicara", descricao: "Xícara", pesoPorUnidade: 225 }
    ]
  },
  "queijo minas": { calorias: 264, proteina: 17.4, carbo: 3.0, gordura: 20.6 },
  "queijo prato": { calorias: 360, proteina: 26.6, carbo: 0.0, gordura: 28.9 },
  "ricota": { calorias: 140, proteina: 8.1, carbo: 3.4, gordura: 10.9 },
  "cream cheese": { calorias: 321, proteina: 8.2, carbo: 2.8, gordura: 31.7 },
  "manteiga": { calorias: 760, proteina: 0.6, carbo: 0.1, gordura: 84.0 },

  // === OLEAGINOSAS E SEMENTES ===
  "amêndoas": { calorias: 579, proteina: 21, carbo: 22, gordura: 50 },
  "castanha do pará": { calorias: 656, proteina: 14, carbo: 12, gordura: 67 },
  "castanha de caju": { calorias: 570, proteina: 15.3, carbo: 29.1, gordura: 46.3 },
  "nozes": { calorias: 654, proteina: 15.2, carbo: 13.7, gordura: 65.2 },
  "amendoim": { calorias: 544, proteina: 27.2, carbo: 20.3, gordura: 43.9 },
  "chia": { calorias: 486, proteina: 17, carbo: 42, gordura: 31 },
  "linhaça": { calorias: 534, proteina: 18, carbo: 29, gordura: 42 },
  "girassol": { calorias: 584, proteina: 19.3, carbo: 24.4, gordura: 51.5 },
  "gergelim": { calorias: 565, proteina: 18.2, carbo: 21.2, gordura: 49.7 },

  // === ÓLEOS E GORDURAS ===
  "azeite de oliva": { calorias: 884, proteina: 0, carbo: 0, gordura: 100 },
  "óleo de soja": { calorias: 884, proteina: 0, carbo: 0, gordura: 100 },
  "óleo de milho": { calorias: 884, proteina: 0, carbo: 0, gordura: 100 },
  "óleo de coco": { calorias: 862, proteina: 0, carbo: 0, gordura: 99.9 },
  "banha de porco": { calorias: 898, proteina: 0, carbo: 0, gordura: 100 },

  // === PÃES E MASSAS ===
  "pão integral": { calorias: 247, proteina: 13, carbo: 41, gordura: 4.2 },
  "pão francês": { calorias: 300, proteina: 9.4, carbo: 58.6, gordura: 3.1 },
  "pão de forma": { calorias: 253, proteina: 8.2, carbo: 48.7, gordura: 3.1 },
  "pão de centeio": { calorias: 240, proteina: 8.2, carbo: 48.2, gordura: 1.7 },
  "macarrão cru": { calorias: 373, proteina: 12.5, carbo: 74.7, gordura: 1.8 },
  "macarrão cozido": { calorias: 102, proteina: 3.3, carbo: 20.3, gordura: 0.5 },
  "lasanha": { calorias: 135, proteina: 6.4, carbo: 16.0, gordura: 5.5 },

  // === SUPLEMENTOS ===
  "whey protein": { calorias: 412, proteina: 80, carbo: 7, gordura: 8 },
  "whey isolado": { calorias: 395, proteina: 90, carbo: 1, gordura: 1 },
  "caseína": { calorias: 373, proteina: 78, carbo: 5.4, gordura: 1.8 },
  "albumina": { calorias: 381, proteina: 81, carbo: 4, gordura: 0.5 },
  "creatina": { calorias: 0, proteina: 0, carbo: 0, gordura: 0 },
  "bcaa pó": { calorias: 240, proteina: 60, carbo: 0, gordura: 0 },
  "maltodextrina": { calorias: 380, proteina: 0, carbo: 95, gordura: 0 },
  "dextrose": { calorias: 380, proteina: 0, carbo: 95, gordura: 0 },
  "hipercalórico": { calorias: 376, proteina: 15, carbo: 75, gordura: 2.5 },

  // === BEBIDAS ===
  "água": { calorias: 0, proteina: 0, carbo: 0, gordura: 0 },
  "café": { calorias: 2, proteina: 0.3, carbo: 0, gordura: 0 },
  "chá verde": { calorias: 0, proteina: 0, carbo: 0, gordura: 0 },
  "refrigerante": { calorias: 42, proteina: 0, carbo: 10.6, gordura: 0 },
  "suco de laranja": { calorias: 45, proteina: 0.7, carbo: 10.4, gordura: 0.2 },
  "cerveja": { calorias: 43, proteina: 0.5, carbo: 3.6, gordura: 0 },
  "vinho tinto": { calorias: 85, proteina: 0.1, carbo: 2.6, gordura: 0 },

  // === DOCES E AÇÚCARES ===
  "açúcar cristal": { calorias: 387, proteina: 0, carbo: 99.9, gordura: 0 },
  "mel": { calorias: 309, proteina: 0.4, carbo: 84.4, gordura: 0 },
  "chocolate amargo": { calorias: 479, proteina: 12.9, carbo: 28.4, gordura: 35.4 },
  "chocolate ao leite": { calorias: 535, proteina: 7.3, carbo: 59.0, gordura: 29.7 },

  // === TEMPEROS E CONDIMENTOS ===
  "sal": { calorias: 0, proteina: 0, carbo: 0, gordura: 0 },
  "vinagre": { calorias: 4, proteina: 0, carbo: 0.6, gordura: 0 },
  "mostarda": { calorias: 67, proteina: 4.4, carbo: 5.8, gordura: 3.3 },
  "ketchup": { calorias: 97, proteina: 1.1, carbo: 25.8, gordura: 0.1 },
  "maionese": { calorias: 681, proteina: 1.3, carbo: 0.6, gordura: 75.0 },

  // === ALIMENTOS PROCESSADOS ===
  "salsicha": { calorias: 296, proteina: 11.5, carbo: 1.4, gordura: 27.1 },
  "mortadela": { calorias: 314, proteina: 14.2, carbo: 1.7, gordura: 27.4 },
  "hambúrguer": { calorias: 293, proteina: 17.0, carbo: 3.0, gordura: 23.8 },
  "nuggets": { calorias: 296, proteina: 15.5, carbo: 13.9, gordura: 20.6 }
};

// Função para buscar alimentos
export function buscarAlimentos(termo) {
  const termoLower = termo.toLowerCase();
  return Object.keys(alimentosDatabase)
    .filter((alimento) => alimento.includes(termoLower))
    .map((alimento) => ({
      nome: alimento,
      ...alimentosDatabase[alimento],
    }));
}

// Função para obter alimento específico
export function obterAlimento(nome) {
  const nomeKey = nome.toLowerCase();
  return alimentosDatabase[nomeKey]
    ? {
        nome: nomeKey,
        ...alimentosDatabase[nomeKey],
      }
    : null;
}

// Função para obter todos os alimentos
export function obterTodosAlimentos() {
  return Object.keys(alimentosDatabase).map((alimento) => ({
    nome: alimento,
    ...alimentosDatabase[alimento],
  }));
}

// Categorias de alimentos para facilitar a busca
export const categorias = {
  cereais: ["arroz integral", "arroz branco", "aveia", "quinoa"],
  leguminosas: ["feijão preto", "feijão carioca", "lentilha", "grão de bico"],
  proteinas: [
    "frango grelhado",
    "peito de frango",
    "carne bovina magra",
    "peixe tilápia",
    "salmão",
    "ovo cozido",
  ],
  vegetais: [
    "brócolis",
    "espinafre",
    "abobrinha",
    "cenoura",
    "tomate",
    "alface",
    "pepino",
  ],
  frutas: [
    "banana",
    "maçã",
    "manga",
    "laranja",
    "morango",
    "uva",
    "melancia",
    "abacaxi",
  ],
  gorduras: [
    "abacate",
    "azeite de oliva",
    "amêndoas",
    "castanha do pará",
    "coco",
  ],
  laticínios: ["leite desnatado", "iogurte grego", "queijo cottage"],
  tuberculos: ["batata doce", "batata inglesa"],
  sementes: ["chia", "linhaça"],
  paes: ["pão integral"],
};

export function buscarPorCategoria(categoria) {
  const alimentosCategoria = categorias[categoria.toLowerCase()] || [];
  return alimentosCategoria.map((alimento) => ({
    nome: alimento,
    ...alimentosDatabase[alimento],
  }));
}
